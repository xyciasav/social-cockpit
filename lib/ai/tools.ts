...
export const assistantTools: AssistantTool[] = [
  ...
  { kind: "draft_write", name: "create_post_drafts", description: "Create fully written post drafts. Every result enters the approval queue; this never schedules or publishes.", parameters: object({ campaignId: { type: "string" }, posts: { type: "array", minItems: 1, maxItems: 12, items: { type: "object", properties: { caption: { type: "string", minLength: 1, maxLength: 5000 }, proposedAt: { type: ["string","null"] }, contentType: { type: "string" }, tone: { type: "string" }, cta: { type: "string" } }, required: ["caption","proposedAt","contentType","tone","cta"], additionalProperties: false } }, useAnalytics: { type: "boolean" } }, ["posts","useAnalytics"]) },
