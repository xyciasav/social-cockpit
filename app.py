...
def generate():
 x=request.get_json(force=True);count=max(1,min(30,int(x.get("count",1))));selected=x.get("library_ids",[]);tone_id=x.get("tone_id")
 lib=rows(f"SELECT category,title,details,url,filename FROM library WHERE id IN ({','.join('?'*len(selected))})",selected) if selected else []
 ...
 try:
  ...
  for i,p in enumerate(generated):
   ...
   when=start.timestamp()+(0 if count==1 else span*i/(count-1));scheduled=datetime.fromtimestamp(when,timezone.utc).isoformat();ident=str(uuid.uuid4());c.execute("INSERT INTO drafts VALUES(?,?,?,?,?,?,?,?)",(ident,caption,scheduled,"pending",tone,x.get("subject",""),None,datetime.now(timezone.utc).isoformat()));created.append(ident)
...
def approve(ident):
 ...
 if not d:return jsonify(error="Draft not found"),404
 if not s["buffer_token"] or not s["buffer_channel"]:return jsonify(error="Configure Buffer API key and channel ID in Settings"),400
 query="""mutation CreatePost($text:String!,$channel:ChannelId!,$due:DateTime!){createPost(input:{text:$text,channelId:$channel,schedulingType:automatic,mode:customScheduled,dueAt:$due}){... on PostActionSuccess{post{id text dueAt}} ... on MutationError{message}}}"""
 r=requests.post("https://api.buffer.com",headers={"Authorization":"Bearer "+s["buffer_token"]},json={"query":query,"variables":{"text":d["caption"],"channel":s["buffer_channel"],"due":d["scheduled_at"]}},timeout=60)
 ...
 c.execute("UPDATE drafts SET status='approved',buffer_id=? WHERE id=?",(post.get("post",{}).get("id"),ident));c.commit();c.close();return jsonify(ok=True)
